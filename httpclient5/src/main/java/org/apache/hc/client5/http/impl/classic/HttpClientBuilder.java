...
public class HttpClientBuilder {
     ...
     * </p>
     *
     * @param enable {@code true} to enable virtual-thread transport execution; {@code false} to use classic threads.
     * @return this instance.
     * @since 5.6
     */
    public HttpClientBuilder useVirtualThreads(final boolean enable) {
    ...
    public CloseableHttpClient build() {
        ...
        ExecutorService vtExecToUse = null;
        if (useVirtualThreads) {
            if (virtualThreadExecutor != null) {
                vtExecToUse = virtualThreadExecutor;
            } else if (VirtualThreadSupport.isAvailable()) {
                vtExecToUse = VirtualThreadSupport.newVirtualThreadPerTaskExecutor(virtualThreadNamePrefix);
            }
        }
